-- =====================================================
-- Simplified batch_update_price_history (No Group Upsert)
-- =====================================================
-- This migration simplifies the batch_update_price_history function.
-- Groups are now upserted ONCE in TypeScript before batch processing begins,
-- so we don't need to upsert groups for every single product (much more efficient).
...
BEGIN
  ...
  LOOP
    BEGIN
      ...
      -- NOTE: Groups are now upserted in TypeScript before this function is called
      -- This is much more efficient than upserting the same group thousands of times
...
COMMENT ON FUNCTION batch_update_price_history IS 'Batch updates price history with product upsert and metric calculation. Groups are upserted separately in TypeScript for efficiency.';
