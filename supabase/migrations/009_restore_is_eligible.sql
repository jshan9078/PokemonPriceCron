-- =====================================================
-- Restore is_eligible Logic (with Unsealed Check)
-- =====================================================
-- This migration restores the logic to automatically set is_eligible = TRUE
-- when market_price >= 15.
--
-- ADDITIONALLY, it enforces that "Unsealed" products (rarity IS NULL AND number IS NULL)
-- are NEVER eligible, regardless of price.
...
BEGIN
  ...
  LOOP
    BEGIN
      ...
      -- NOTE: Groups are upserted in TypeScript before this function is called
      ...
      IF NOT v_product_exists THEN
        ...
        INSERT INTO products (
          ...
          -- Unsealed defined as: rarity IS NULL AND number IS NULL
          CASE 
      ...
      -- Product exists - get its price history AND existing attributes for eligibility check
      ...
      -- (Update logic below uses COALESCE(v_rarity, rarity))
      ...
      -- (Logic from 007_fix_24h_change_calculation.sql)
      -- =====================================================
      
      v_current_price := v_price;

      -- Use non-overlapping time windows, starting at 1 day back
...
COMMENT ON FUNCTION batch_update_price_history IS 'Batch updates price history with product upsert, metric calculation, and automatic is_eligible tracking (price >= $15, excluding unsealed)';
