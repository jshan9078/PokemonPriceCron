-- =====================================================
-- Fix 24h Change Calculation Bug
-- =====================================================
-- This migration fixes the bug where 24h change (and all other time-based changes)
-- were always showing 0 because the current day's price was being included in the
-- historical price lookup.
--
-- ROOT CAUSE:
-- The batch_update_price_history function was adding today's price to price_history
-- BEFORE calculating historical metrics. When get_nearest_price_in_range searched
-- for "1 day ago" with range (0, 2), it would find day 0 (today) and return the
-- current price, causing: change = (current - current) / current = 0
--
-- FIX:
-- 1. Move the metric calculations BEFORE adding today's price to price_history
-- 2. Adjust the day ranges to start at 1 instead of 0 to ensure we never match today
--
-- PERFORMANCE IMPACT: None - we're just reordering in-memory JSONB operations
...
BEGIN
  ...
  LOOP
    BEGIN
      ...
      -- =====================================================
      -- FIX: Calculate metrics BEFORE adding today's price
      -- =====================================================
      -- This ensures we're comparing today's price against
      -- actual historical prices, not against itself

      v_current_price := v_price;

      -- Use non-overlapping time windows, starting at 1 day back (not 0!)
      -- 1 day ago: search 1-3 days back (skip day 0 = today)
      v_price_1d_ago := get_nearest_price_in_range(v_price_history, v_date_parsed, 1, 3);

      -- 3 days ago: search 3-5 days back (adjusted to skip overlap)
      v_price_3d_ago := get_nearest_price_in_range(v_price_history, v_date_parsed, 3, 5);

      -- 7 days ago: search 6-10 days back (adjusted from 5-9)
      v_price_7d_ago := get_nearest_price_in_range(v_price_history, v_date_parsed, 6, 10);

      -- 1 month ago: search 26-36 days back (adjusted from 25-35)
      v_price_1m_ago := get_nearest_price_in_range(v_price_history, v_date_parsed, 26, 36);

      -- 3 months ago: search 85-105 days back (adjusted from 80-100)
      v_price_3m_ago := get_nearest_price_in_range(v_price_history, v_date_parsed, 85, 105);

      -- 6 months ago: search 175-195 days back (adjusted from 170-190)
      v_price_6m_ago := get_nearest_price_in_range(v_price_history, v_date_parsed, 175, 195);

      -- 1 year ago: search 355-385 days back (adjusted from 350-380)
      v_price_1y_ago := get_nearest_price_in_range(v_price_history, v_date_parsed, 355, 385);

      -- =====================================================
      -- Now append today's price to price_history
      -- =====================================================
      -- This happens AFTER metrics calculation, so today's price
      -- won't interfere with historical lookups
...
COMMENT ON FUNCTION batch_update_price_history IS 'Batch updates price history (NUMERIC VALUES ONLY) with UPSERT support, low/high price tracking, and CORRECT 24h change calculation';
